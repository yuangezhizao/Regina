...
def getAppFilePaths(appName):
    ...
    file_paths.source_file = os.path.join(file_paths.folder, toSnakeCase(appName) + '.cpp')

    file_paths.header_file = os.path.join(file_paths.folder, toSnakeCase(appName) + '.h')
...
def createAppFiles(appName, appFilePaths):
    ...
    # 替换类名
    content_source_file = content_source_file.replace('AppTemplate', appName)
    content_header_file = content_header_file.replace('AppTemplate', appName)

    # 替换文件名
    content_source_file = content_source_file.replace('app_template', toSnakeCase(appName))
    content_header_file = content_header_file.replace('app_template', toSnakeCase(appName))

    # 替换日期
    content_source_file = content_source_file.replace('<date></date>', datetime.now().strftime('%Y-%m-%d'))
    content_header_file = content_header_file.replace('<date></date>', datetime.now().strftime('%Y-%m-%d'))
...
def installApp(appName):
    print(f'> install app {appName} into mooncake')
    ...
    content_app_install_cb_file = content_app_install_cb_file.replace(
        app_install_tag,
        "mooncake::GetMooncake().installApp(std::make_unique<{}>());\n    {}".format(appName, app_install_tag)
    )
