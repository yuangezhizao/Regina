...
class Oled : public lgfx::LGFX_Device
{
    /*
    ...
    lgfx::Panel_SSD1312 _panel_instance;
...
public:
    ...
    Oled()
    {
        {                                      // バス制御の設定を行います。
            ...
            cfg.pin_sclk = HAL_PIN_OLED_SCL; // SPIのSCLKピン番号を設定
            cfg.pin_mosi = HAL_PIN_OLED_SDA; // SPIのMOSIピン番号を設定
            cfg.pin_miso = GPIO_NUM_NC;      // SPIのMISOピン番号を設定 (-1 = disable)
            cfg.pin_dc = HAL_PIN_OLED_DC;    // SPIのD/Cピン番号を設定  (-1 = disable)
        ...
        {                                        // 表示パネル制御の設定を行います。
            ...
            cfg.pin_cs = HAL_PIN_OLED_CS;   // CSが接続されているピン番号   (-1 = disable)
            cfg.pin_rst = HAL_PIN_OLED_RST; // RSTが接続されているピン番号  (-1 = disable)
            cfg.pin_busy = GPIO_NUM_NC;     // BUSYが接続されているピン番号 (-1 = disable)

            // ※
            // 以下の設定値はパネル毎に一般的な初期値が設定されていますので、不明な項目はコメントアウトして試してみてください。

            cfg.panel_width = HAL_SCREEN_WIDTH;   // 実際に表示可能な幅
            cfg.panel_height = HAL_SCREEN_HEIGHT; // 実際に表示可能な高さ
